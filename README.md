...
- 공개 배포용 기본 구성은 `READ_SMS` 권한을 포함하지 않습니다.
- 거래 알림, 파일 가져오기, 수동 입력 데이터는 기본적으로 기기 내부 DB에만 저장됩니다.
- 자동 백업은 기본 비활성화이며, 백업이 필요하면 CSV 내보내기를 사용합니다.

## 빌드 참고

- Windows는 `C:/temp/moneymind-build`, macOS/Linux는 시스템 임시 디렉터리 아래에 Gradle 빌드 산출물을 생성합니다.
...
3. 백업/복원 자동화(스케줄 백업, 선택적 클라우드 연동) 추가
